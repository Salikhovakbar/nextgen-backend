...
if(id && await attendance.get(id)) return res.send({status:200, data: await attendance.get(id)})
    ...
    POST_ATTENDANCE: async (req, res) => {
        try{
const { group_id, teacher_id, firstname, lastname} = req.body
if( !group_id || !teacher_id || !firstname || !lastname) throw new Error("The data is not full")
else{
